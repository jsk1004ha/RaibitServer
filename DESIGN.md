...
- `/status`는 현재 실행 중인 Dashboard 이미지에 기록된 GitHub 커밋을 12자리 축약 SHA로 표시하고, 검증된 전체 SHA의 GitHub 커밋 페이지로 연결한다.
- 배포 메타데이터가 없거나 형식이 올바르지 않으면 임의 링크를 만들지 않고 `확인 불가`로 표시한다.
- 배포 버전은 상태 구성 요소와 분리된 작은 모노스페이스 메타 정보로 두며 모바일에서는 자동 갱신·확인 시각과 함께 세로로 재배치한다.
