...
export const GitHubRepository = z.object({ id, installationId: id, githubRepoId: id, owner: z.string(), name: z.string(), fullName: z.string(), normalizedIdentity: z.string(), defaultBranch: z.string(), private: z.boolean(), accessState: z.enum(['ACCESSIBLE', 'REVOKED']), generation: z.number().int().nonnegative() }).catchall(json);
export const GitHubCatalogPage = z.object({ installationId: id, generation: z.number().int().nonnegative(), refreshStatus: z.enum(['IDLE', 'REFRESHING', 'STALE']), lastSuccessfulSyncAt: z.iso.datetime().nullable(), staleAt: z.iso.datetime().nullable(), repositories: z.array(GitHubRepository).max(50), nextCursor: z.string().nullable() }).strict();
