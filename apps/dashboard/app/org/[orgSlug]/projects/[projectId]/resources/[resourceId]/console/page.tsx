  ...
  return (
    <ConsoleShell active="projects" orgValue={orgSlug} orgRouteValue={orgSlug} projectValue={projectId} projectId={projectId}>
      <section className="mx-auto flex w-full max-w-6xl min-w-0 flex-col gap-6 px-4 py-6 md:px-6 md:py-8" data-od-id="resource-console">
        ...
        {view === 'provision' ? <form id="provisioning" method="post" action={apiAction(`/resources/${resourceId}/provision`, state.context)}><input type="hidden" name="_returnTo" value={`${base}?view=provision`} /><input type="hidden" name="dryRun" value="true" /><Card><CardHeader className="border-b"><CardTitle><h2>프로비저닝</h2></CardTitle><CardDescription>실제 변경 전에 공급자 변경 계획을 만듭니다.</CardDescription></CardHeader><CardContent className="flex flex-col gap-4"><p className="text-sm text-muted-foreground">이 작업은 <code className="font-mono">dryRun=true</code>로 제출되어 현재 상태를 직접 바꾸지 않습니다.</p><button className={buttonVariants()} type="submit">계획 만들기</button></CardContent></Card></form> : null}
        {view === 'connection' ? <section aria-labelledby="connection-heading"><Card><CardHeader className="border-b"><CardTitle><h2 id="connection-heading">서비스 연결</h2></CardTitle><CardDescription>리소스 연결 정보를 서비스 환경 변수로 연결합니다.</CardDescription></CardHeader><CardContent><form id="connection" method="post" action={apiAction(`/resources/${resourceId}/attach`, state.context)}><input type="hidden" name="_returnTo" value={`${base}?view=connection`} /><FieldGroup><Field><FieldLabel htmlFor="service-id">서비스 ID</FieldLabel><Input id="service-id" name="serviceId" placeholder="service id" required autoComplete="off" /></Field><Field><FieldLabel htmlFor="env-prefix">환경 변수 접두사</FieldLabel><Input id="env-prefix" name="envPrefix" placeholder="선택 사항: ENV_PREFIX" autoCapitalize="characters" autoComplete="off" /><FieldDescription>비워 두면 엔진 기본 접두사를 사용합니다.</FieldDescription></Field><button className={buttonVariants()} type="submit">서비스에 연결</button></FieldGroup></form></CardContent></Card></section> : null}
      </section>
    </ConsoleShell>
  );
}
