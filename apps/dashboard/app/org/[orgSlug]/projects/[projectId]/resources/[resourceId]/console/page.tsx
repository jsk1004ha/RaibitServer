...
import { ResourceBackupActions } from '@/components/resource-backup-actions';
import { OperationSubmit } from '@/components/operation-submit';
import { ResourceBackupViewSchema } from '@raibitserver/schemas';
import type { ResourceBackupView } from '@raibitserver/schemas';
...
export default async function ResourceConsolePage({ params, searchParams }: { params: Promise<{ orgSlug: string; projectId: string; resourceId: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const schemaDocument = state.schema?.schema && typeof state.schema.schema === 'object' && !Array.isArray(state.schema.schema)
    ...
    : {};
  const backupRows = Array.isArray(state.backups?.backups)
    ? state.backups.backups.flatMap((entry: unknown) => {
      const parsed = ResourceBackupViewSchema.safeParse(entry);
      return parsed.success ? [parsed.data] : [];
    })
    : [];
  const backupRestoreKeys = Object.fromEntries(backupRows.map((backup: ResourceBackupView) => [backup.id, `restore-${randomUUID()}`]));
  const backupsUnavailable = state.loadErrors.some((issue) => issue.label === '백업');
  ...
  return (
    <ConsoleShell active="projects" orgValue={orgSlug} orgRouteValue={orgSlug} projectValue={projectId} projectId={projectId}>
      <section className="mx-auto flex w-full max-w-6xl min-w-0 flex-col gap-6 px-4 py-6 md:px-6 md:py-8" data-od-id="resource-console">
        ...
        {view === 'backups' ? <ResourceBackupActions createAction={apiAction(`/resources/${resourceId}/backups`, state.context)} initialBackups={backupRows} initialCreateKey={`backup-${randomUUID()}`} initialLoadFailed={backupsUnavailable} initialRestoreKeys={backupRestoreKeys} listAction={apiAction(`/resources/${resourceId}/backups`, state.context)} returnTo={`${base}?view=backups`} /> : null}
