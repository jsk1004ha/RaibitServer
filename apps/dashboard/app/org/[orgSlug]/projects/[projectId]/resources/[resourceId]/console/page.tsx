  ...
  return (
    <ConsoleShell active="projects" orgValue={orgSlug} orgRouteValue={orgSlug} projectValue={projectId} projectId={projectId}>
      <section className="mx-auto flex w-full max-w-6xl min-w-0 flex-col gap-6 px-4 py-6 md:px-6 md:py-8" data-od-id="resource-console">
        ...
        {view === 'backups' ? <ResourceBackupActions createAction={apiAction(`/resources/${resourceId}/backups`, state.context)} initialBackups={backupRows} initialLoadFailed={backupsUnavailable} listAction={apiAction(`/resources/${resourceId}/backups`, state.context)} returnTo={`${base}?view=backups`} /> : null}
