...
export default async function DeploymentDetailPage({ params, searchParams }: { params: Promise<{ orgSlug: string; projectId: string; deploymentId: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const requestIdempotencyKey = snapshotVersion === null ? null : randomUUID();
          ...
          <Card>
            ...
            <CardContent className="flex flex-col gap-raibit-lg">
              <div className="flex flex-wrap gap-raibit-sm">
                <OperationSubmit action={apiAction(`/deployments/${encodedDeploymentId}/retry`, context)} disabled={!retryAllowed} pendingLabel="재시도 요청을 확인하고 있습니다." returnTo={`${base}?view=overview`} submitClassName={buttonVariants({ variant: 'outline', size: 'sm' })} submitLabel="실패한 배포 다시 시도">{snapshotVersion !== null && requestIdempotencyKey !== null ? <><input name="snapshotVersion" type="hidden" value={String(snapshotVersion)} /><input name="requestIdempotencyKey" type="hidden" value={requestIdempotencyKey} /></> : null}</OperationSubmit>
                <OperationSubmit action={apiAction(`/services/${String(detail.serviceId || '')}/redeploy`, context)} disabled={!redeployAllowed} pendingLabel="재배포 요청을 확인하고 있습니다." returnTo={`${base}?view=overview`} submitClassName={buttonVariants({ variant: 'outline', size: 'sm' })} submitLabel="현재 구성으로 재배포">{snapshotVersion !== null && requestIdempotencyKey !== null ? <><input name="snapshotVersion" type="hidden" value={String(snapshotVersion)} /><input name="requestIdempotencyKey" type="hidden" value={requestIdempotencyKey} /></> : null}</OperationSubmit>
