          ...
          <section className="card">
            ...
            <table className="table" style={{ marginTop: 12 }}><thead><tr><th>사용자</th><th>역할 / 계정</th><th>상태</th><th>작업</th></tr></thead><tbody>
              {state.users.length ? state.users.map((user: any) => <tr key={user.id}>
                ...
                <td className="table-actions"><form method="post" action={apiAction(`/admin/users/${user.id}/approve`, state.context)} className="inline-actions"><input type="hidden" name="accountType" value="CLUB_MEMBER" /><button type="submit">클럽 회원으로 승인</button></form><form method="post" action={apiAction(`/admin/users/${user.id}/approve`, state.context)} className="inline-actions" style={{ marginTop: 8 }}><input type="hidden" name="accountType" value="NON_CLUB" /><button type="submit">일반 사용자로 승인</button></form><form method="post" action={apiAction(`/admin/users/${user.id}/reject`, state.context)} className="inline-actions danger-zone" style={{ marginTop: 8 }}><label className="confirmation-control"><input type="checkbox" name="confirmed" value="true" required /><span><strong>{user.email || user.name}</strong> 계정 거절 확인</span></label><button className="btn-danger" type="submit">거절</button></form></td>
              </tr>) : <tr><td colSpan={4}>표시할 사용자가 없습니다.</td></tr>}
            </tbody></table>
          </section>
          <aside className="stack">
            <section className="card"><h2>할당량 편집</h2><p className="muted" style={{ marginTop: 8 }}>대상 사용자를 펼쳐 변경하세요. 변경 전후 값과 작업자가 감사 로그에 남습니다.</p><div className="stack" style={{ marginTop: 12 }}>{state.users.length ? state.users.map((user: any) => <details className="quota-editor" key={user.id}><summary><strong>{user.email || user.name}</strong><span className="muted mono">{user.id}</span></summary><form method="post" action={apiAction(`/admin/users/${user.id}/quota`, state.context)} className="form-grid" style={{ marginTop: 12 }}><input type="hidden" name="targetUserId" value={user.id} /><label>프로젝트 수 <input name="maxProjects" inputMode="numeric" placeholder="최대 프로젝트 수" /></label><label>서비스 수 <input name="maxServices" inputMode="numeric" placeholder="최대 서비스 수" /></label><button type="submit">할당량 저장</button></form></details>) : <p className="muted">편집할 사용자가 없습니다.</p>}</div></section>
