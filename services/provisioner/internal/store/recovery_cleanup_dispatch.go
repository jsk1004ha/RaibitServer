...
func (s *PostgresStore) NextRecoveryCleanup(ctx context.Context) (*RecoveryIdentity, error) {
 ...
 WHERE b."formatVersion"=1
   ...
   AND (b.status='DELETING'
     OR NOT EXISTS (SELECT 1 FROM "ResourceRecoveryAttempt" a WHERE a."backupId"=b.id)
     OR EXISTS (SELECT 1 FROM "ResourceRecoveryAttempt" a WHERE a."backupId"=b.id AND a."cleanupPending"))
