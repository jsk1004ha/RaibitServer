...
export async function loadBindings(options) { return load({ ...options, unsafeFixture: false }); }
export async function loadBindingsFixtureUnsafe(options) { return load({ ...options, unsafeFixture: true }); }

async function append(options) {
  const required = ['runDirectory', 'identity', 'role', 'bindingId', 'payload', 'createdAt'];
  if (!isRecord(options) || Object.keys(options).some((key) => ![...required, 'parsePayload', 'unsafeFixture', 'writer'].includes(key))
...
export async function bindingJournalSnapshot(options) { return snapshot({ ...options, unsafeFixture: false }); }
export async function bindingJournalSnapshotFixtureUnsafe(options) { return snapshot({ ...options, unsafeFixture: true }); }
