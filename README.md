...
7. **GitHub App/OAuth 연결**
   ...
   - BFF의 state/verifier 쿠키는 Secure·HttpOnly·SameSite=Lax·host-only이며 callback 경로에만 전송됩니다. 로컬 브라우저 검증에도 HTTPS가 필요합니다. 모든 callback 결과에서 두 쿠키를 지우며, 브라우저 쿠키 확인 후에만 API 소비를 요청합니다. API 소스는 실제 socket peer를 사용합니다. BFF 경유 요청의 peer는 BFF이므로 브라우저 구분은 별도 쿠키/verifier가 맡습니다. forwarded IP 신뢰 정책과 추가 남용 방지는 별도 단계이며 여기서 proxy trust를 활성화하지 않습니다.
