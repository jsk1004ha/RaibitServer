...
### 지속 실행 서비스와 체크포인트 저장소

서비스 설정의 영구 저장소는 `sizeGi` 1–100과 `/data` 아래의 안전한 `mountPath`를 받습니다. `web`, `private`, `worker`만 지원하며, 기본 StorageClass의 Filesystem/ReadWriteOnce PVC를 사용합니다. 해당 서비스는 인스턴스 1개, Recreate 배포, UID/fsGroup 10001, 종료 유예 300초로 실행됩니다. 롤링 업데이트·자동 확장·PR 미리보기와 동시에 사용할 수 없습니다.

CPU·메모리는 인증된 사용자 할당량 안에서 설정하며 저장소 용량은 기존 `maxObjectStorageMb`에 합산합니다. 기존 볼륨의 크기·경로·서비스 이름은 이 단계에서 변경하지 않습니다. 재배포는 PVC를 재사용하며 일반 리소스 정리는 PVC를 제거하지 않습니다. 영구 저장소가 있는 서비스의 개별 삭제는 별도 저장소 수명주기 구현 전까지 차단됩니다. 프로젝트/namespace 삭제는 볼륨도 삭제할 수 있으므로 반드시 먼저 외부 백업을 확보해야 합니다. PVC는 백업이 아닙니다.

운영 전 기본 StorageClass, 여유 용량, PVC Bound 상태, UID 10001의 쓰기 권한을 확인합니다. 체크포인트를 기록한 뒤 해당 서비스만 재시작하여 동일 파일과 학습 단계가 복원되는지 확인합니다. 플랫폼 단위 테스트나 매니페스트 생성 성공만으로 실제 데이터 보존을 입증하지 않습니다.
